...
def create_segmentation_visualization(image, objects, labeled_masks=None, metadata=None):
    ...
    # First, draw segmentation masks if available from FastSAM
    if labeled_masks is not None and metadata is not None:
        # Get unique mask IDs (excluding 0 which is background)
        mask_ids = [mid for mid in np.unique(labeled_masks) if mid > 0]
    ...
    # Now draw all object bounding boxes and their surface masks
    if objects:
        # Generate colors for each object
        obj_colors = get_unique_colors(len(objects))
        
        # Create a new overlay for object surface masks
        obj_mask_overlay = np.zeros_like(seg_vis, dtype=np.uint8)
        mask_drawn = False
        ...
        for i, obj in enumerate(objects):
            ...
            # Draw object surface mask if available
            if hasattr(obj, 'mask') and obj.mask is not None:
                mask = obj.mask
                if isinstance(mask, np.ndarray) and mask.shape[:2] == image.shape[:2]:
                    # If mask is a binary mask
                    mask_indices = mask > 0 if mask.dtype == bool else mask > 0.5
                    obj_mask_overlay[mask_indices] = color
                    mask_drawn = True
                elif isinstance(mask, list) and len(mask) > 0:
                    # If mask is a list of points (polygon)
                    try:
                        mask_points = np.array(mask, dtype=np.int32)
                        # Draw filled polygon
                        cv2.fillPoly(obj_mask_overlay, [mask_points], color)
                        mask_drawn = True
                    except Exception as e:
                        print(f"Error drawing polygon mask: {e}")
            ...
            if obj.components:
                for comp_id, comp in obj.components.items():
                    ...
                    if hasattr(comp, 'mask') and comp.mask is not None:
                        comp_mask = comp.mask
                        if isinstance(comp_mask, np.ndarray) and comp_mask.shape[:2] == image.shape[:2]:
                            # If mask is a binary mask
                            mask_indices = comp_mask > 0 if comp_mask.dtype == bool else comp_mask > 0.5
                            obj_mask_overlay[mask_indices] = comp_color
                            mask_drawn = True
                        elif isinstance(comp_mask, list) and len(comp_mask) > 0:
                            # If mask is a list of points (polygon)
                            try:
                                mask_points = np.array(comp_mask, dtype=np.int32)
                                # Draw filled polygon
                                cv2.fillPoly(obj_mask_overlay, [mask_points], comp_color)
                                mask_drawn = True
                            except Exception as e:
                                print(f"Error drawing component polygon mask: {e}")
        
        # Blend the object mask overlay with the image if any masks were drawn
        if mask_drawn:
            seg_vis = cv2.addWeighted(seg_vis, 0.7, obj_mask_overlay, 0.3, 0)
...
def main():
    ...
    try:
        ...
        point_method = "contours"  # Default method for point detection
