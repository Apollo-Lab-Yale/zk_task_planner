#!/usr/bin/env python3
"""
Test script for using Intel Realsense camera with Hybrid YOLO model
combining YOLOWorld detection with YOLO-E segmentation for real-time detection.
"""
...
# Add the parent directory to sys.path to import the HybridYOLO class
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))
from src.vision.object_detection.yolo import HybridYOLO
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Test Realsense camera with Hybrid YOLO')
    parser.add_argument('--yolo-world-model', type=str, default='yolov8x-worldv2.pt', 
                        help='YOLOWorld model path or name')
    parser.add_argument('--fastsam-model', type=str, default='FastSAM-x.pt', 
                        help='FastSAM model path or name')
    ...
    try:
        ...
        print(f"Loaded Hybrid YOLO - YOLOWorld: {args.yolo_world_model}, FastSAM: {args.fastsam_model}")
        ...
        print(f"Error loading Hybrid YOLO model: {e}")
        camera.release()
        return
    
    # Create windows for display
    cv2.namedWindow('Hybrid YOLO Detection', cv2.WINDOW_AUTOSIZE)
    ...
    try:
        while True:
            ...
            # Run inference with Hybrid YOLO
            detections = model.detect_objects(color_image)
            ...
            for det in detections:
                ...
                # Apply mask overlay
                color = (0, 255, 0)
                annotated_frame = model.apply_mask_overlay(annotated_frame, mask, color, alpha=0.3)
            ...
            cv2.imshow('Hybrid YOLO Detection', annotated_frame)
